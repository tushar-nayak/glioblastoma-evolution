...
class Glioma3DDataset(Dataset):
    def __init__(self, root_dir, modalities, resize_dim=(64, 64, 64)):
        ...
        # Scan for valid weeks (files must exist for all modalities)
    ...
    def _load_volume(self, week):
        ...
        for mod in self.modalities:
            ...
            # Normalize to 0-1 (Crucial for PDE stability)
    ...
    def __getitem__(self, idx):
        ...
        return {
            ...
            'y_t1': self._load_volume(week_t1)[0:1], # Target is just FLAIR (Tumor Conc.)
...
class ParameterEstimator3D(nn.Module):
    ...
    def __init__(self, in_channels=4):
        super().__init__()
        # 3D Encoder to extract biological properties from images
    ...
    def forward(self, x):
        ...
        # --- CRITICAL FIX 1: INCREASED DIFFUSION CAP ---
        # Previous: * 0.1 (Too low, caused saturation in Figure 2)
        # New:      * 0.5 (Allows faster diffusion)
...
class ReactionDiffusionSolver(nn.Module):
    """
    Differentiable PDE Solver for Fisher-Kolmogorov Equation
    """
...
class GliomaPhysicsModel(nn.Module):
    ...
    def forward(self, x_t0, dt):
        ...
        c_t0 = x_t0[:, 0:1] # Use FLAIR channel as tumor concentration
...
def train_physics_model():
    ...
    print("\n🚀 Starting Physics-Informed Training (v3 - Contrast Boost)...")
    loss_history = []
    ...
    for epoch in range(CONFIG['epochs']):
        ...
        for batch in loader:
            ...
            # 2. Smoothness (DRASTICALLY REDUCED)
            # We want to allow edges/structure to form.
            # Reduced from 0.1 to 0.001
            smooth_loss = torch.mean(torch.abs(D_map[:, :, 1:] - D_map[:, :, :-1])) + \
                          torch.mean(torch.abs(rho_map[:, :, 1:] - rho_map[:, :, :-1]))
            
            # 3. Variance Hinge Loss (NEW)
            # Instead of just rewarding variance, we explicitly PENALIZE flatness.
            # If the std_dev of the map is less than 0.05, the loss skyrockets.
            map_std = torch.std(D_map)
            variance_penalty = torch.relu(0.05 - map_std)
            
            # Total Loss
            # fit (1.0) + smooth (0.001) + Variance_Enforcement (10.0)
        ...
        if (epoch + 1) % 10 == 0:
            # We want to see D_range widening (e.g., 0.2 to 0.5), not just shifting up.
...
def visualize_results(model, dataset):
    ...
    # Slice the middle of the 3D volume for 2D visualization
    ...
    # This map should now show structure, not just a red square
